...
public class EnemyBullet : MonoBehaviour
{
	...
	void Start()
	{
		GetComponent<Animation>().Play("stay");
	}
	...
	void WayPointMove()
	{
		...
		{
			//회전
			transform.rotation = Quaternion.Slerp(transform.rotation,Quaternion.LookRotation(WayPoint1.transform.position - transform.position), 1);
			//이동
	...
	void Attack()
	{
		GetComponent<Animation>().CrossFade("shoot", 0.5f); //애니메이션 변경
		//돌아보는 방향을 플레이어 쪽으로
